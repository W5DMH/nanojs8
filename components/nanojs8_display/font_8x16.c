/*
 * font_8x16.c — IBM CP437-style 8x16 bitmap font, printable ASCII subset
 *
 * Source: Linux kernel lib/fonts/font_8x16.c (chars 0x20-0x7E).
 * The Linux kernel font is licensed GPL-2.0, compatible with our project's
 * GPL-3.0 license. The underlying glyph designs derive from the IBM PC
 * BIOS character ROM (1981), which has been in the public domain for
 * decades and was incorporated into the Linux kernel font early on.
 *
 * Format: each character is 16 bytes (rows top-to-bottom), 8 pixels per
 * row, MSB-first (bit 7 = leftmost pixel, bit 0 = rightmost).
 *
 * Indexed by (ascii_code - 0x20). Range: 0x20 space through 0x7E tilde
 * (95 characters). Codes outside this range render as a filled block
 * by the drawing function.
 */
#include <stdint.h>

const uint8_t nanojs8_font_8x16[95][16] = {
    ...
    /* 0x5C 'backslash' */ { 0x00, 0x00, 0x00, 0x80, 0xc0, 0xe0, 0x70, 0x38, 0x1c, 0x0e, 0x06, 0x02, 0x00, 0x00, 0x00, 0x00 },
    ...
    /* 0x77 'w' */ { 0x00, 0x00, 0x00, 0x00, 0x00, 0xc6, 0xc6, 0xc6, 0xd6, 0xd6, 0xfe, 0x6c, 0x00, 0x00, 0x00, 0x00 },
