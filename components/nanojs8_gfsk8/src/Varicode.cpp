...
std::string Varicode::extendedChars() {
    // NanoJS8 L7.4b: stubbed. Original body iterated JSC::prefix[] which
    // we no longer define (TX-only table, see components/nanojs8_gfsk8/
    // src/JSC.cpp for the full RX-only rationale). This function has no
    // callers in the gfsk8 codebase so stubbing it doesn't break anything.
...
#define ALLOW_FORCE_IDENTIFY 1
    ...
    for (std::string line : lines) {
        ...
        // L7.11g.7-fix3: defensive non-progress guard. The while-loop
        // below only advances `line` if at least one of l/o/n/m comes
        // back > 0 from the four packers (heartbeat/compound/directed/
        // data). For some inputs (e.g. text starting with a callsign
        // but containing characters not in the data-pack alphabet —
        // commas, certain punctuation) all four can return 0, leaving
        // `line` unchanged and the loop spinning forever, burning a
        // full CPU core silently with no log output. Detect that
        // and bail with whatever was already packed.
        ...
        while (line.size() > 0) {
            ...
            // L7.11g.7-fix3: non-progress guard. If none of useBcn/
            // useCmp/useDir/useDat ran (i.e. all of l, o, n, m came
            // back 0), `line` is unchanged and the loop would spin
            // forever. Break out — caller (text_frame_count) will see
            // a possibly-empty result and log the failure cleanly.
