...
// NanoJS8 L7.4b: PSRAM placement for large globals (see attribute on
// dec_data + specData below). Requires CONFIG_SPIRAM_ALLOW_BSS_SEG_
// EXTERNAL_MEMORY=y in sdkconfig (set in sdkconfig.defaults).
#include "esp_attr.h"
...
//
// NanoJS8 modification: dec_data + specData are placed in PSRAM via
// EXT_RAM_BSS_ATTR. dec_data alone is 1.44 MB (int16_t[720000] +
// DecodeParams); placing it in internal DRAM would either fail to link
// or leave ~50 KB of DRAM free for everything else. PSRAM has 8 MB
// budget with ~6 MB free at L7.3 baseline; 1.44 MB fits comfortably.
// fftw_mutex is small (~16 B) and stays in DRAM where mutex perf matters.

EXT_RAM_BSS_ATTR RxAudioBuffer dec_data;
EXT_RAM_BSS_ATTR RxSpectrum    specData;
std::mutex                     fftw_mutex;
